...
function App() {
  ...
  const [isStudentManageModalOpen, setIsStudentManageModalOpen] =
    useState(false);
  ...
  const toggleRequest = async (dateStr, time) => {
    ...
    const isSlotConfirmed = schedules.some(
      (s) =>
  ...
  const handleScheduleClick = async (schedule) => {
    ...
    if (isTeacher) {
      if (schedule.status === "confirmed") {
        if (
          window.confirm(
            `'${schedule.studentName}' 학생의 예약을 취소하시겠습니까?`,
          )
        ) {
        ...
        if (
          window.confirm(`'${schedule.studentName}' 학생을 확정하시겠습니까?`)
        ) {
      ...
      <div className="flex-1 overflow-auto bg-white">
        <div className="min-w-[768px]">
          <div className="grid grid-cols-8 border-b sticky top-0 bg-white z-10 shadow-sm">
            <div className="p-2 text-center font-bold border-r bg-gray-50 text-gray-500">
              시간
            </div>
            {days.map((day) => (
              <div
                key={day.toString()}
                className={`p-2 text-center border-r font-semibold ${isSameDay(day, new Date()) ? "bg-indigo-50 text-indigo-600" : "text-gray-700"}`}
              >
            ...
            {TIME_SLOTS.map((time) => (
              <div key={time} className="grid grid-cols-8 min-h-[60px]">
                ...
                {days.map((day) => {
                  ...
                  const rawData = schedules.filter(
                    (s) =>
                      s.scheduleDate === dateStr && s.scheduleTime === time,
                  );
                  const cellData = getVisibleSchedules(rawData);

                  const isPast = isPastTime(dateStr, time);
                  let bgColorClass = isPast
                      ...
                      {cellData.map((schedule) => (
                        ...
                        >
                          ...
                          {!isPast && !schedule.isMasked && (
                            <X
                              size={12}
                              className="opacity-0 group-hover:opacity-100 cursor-pointer hover:bg-black/10 rounded"
                            />
                          )}
                        </div>
                      ))}
                      {isTeacher &&
                        ...
                        cellData.length === 0 && (
                          <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100">
                            <div className="bg-indigo-100 text-indigo-600 rounded-full p-1 shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-white">
        <div className="grid grid-cols-7 mb-2">
          {["일", "월", "화", "수", "목", "금", "토"].map((dayName, idx) => (
            <div
              key={idx}
              className={`text-center font-bold ${idx === 0 ? "text-red-500" : "text-gray-600"}`}
            >
              {dayName}
            </div>
          ))}
            ...
            <div key={wIdx} className="grid grid-cols-7 gap-1 h-32">
              {weekRow.map((day) => {
                ...
                return (
                  <div
                    key={day.toString()}
                    className={`border rounded-lg p-2 flex flex-col ${isCurrentMonth ? "bg-white" : "bg-gray-50 text-gray-400"}`}
                  >
                    <div
                      className={`text-sm font-bold mb-1 ${isToday ? "bg-indigo-600 text-white w-6 h-6 rounded-full flex items-center justify-center" : ""}`}
                    >
                      {format(day, "d")}
                    </div>
                    <div className="flex-1 overflow-hidden space-y-1">
                      ...
                      {visibleSchedules.length > 3 && (
                        <div className="text-[10px] text-gray-400 pl-1">
                          + {visibleSchedules.length - 3} more
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="flex w-full md:w-auto justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-2 bg-gray-100 rounded-full p-1">
              <button
                ...
                className={`px-3 md:px-4 py-1.5 rounded-full text-xs md:text-sm font-bold transition-all ${viewMode === "week" ? "bg-white text-indigo-600 shadow-sm" : "text-gray-500"}`}
              >
                주간
              </button>
              <button
                onClick={() => setViewMode("month")}
                className={`px-3 md:px-4 py-1.5 rounded-full text-xs md:text-sm font-bold transition-all ${viewMode === "month" ? "bg-white text-indigo-600 shadow-sm" : "text-gray-500"}`}
              >
                월간
              </button>
            </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
            <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg border w-full md:w-auto justify-between md:justify-start">
              ...
              <span className="font-bold text-base md:text-lg text-gray-700 min-w-[120px] text-center">
                ...
                {viewMode === "week" && (
                  <span className="text-xs md:text-sm font-normal text-gray-500 ml-1">
                    (
                    {format(startOfWeek(currentDate, { weekStartsOn: 1 }), "d")}
              ...
              <button
                ...
                className="ml-2 text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded font-bold hover:bg-indigo-200 whitespace-nowrap"
              >
                오늘
              </button>
            </div>
            ...
            {!isTeacher ? (
              <div className="flex items-center gap-2 w-full md:w-auto justify-end">
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-gray-500 hover:text-indigo-600 text-sm font-bold transition-colors whitespace-nowrap"
                >
                  <Lock size={14} /> 선생님
                </button>
              </div>
              ...
              <div className="flex items-center gap-3 w-full md:w-auto justify-end relative">
                <span className="text-indigo-600 font-bold flex items-center gap-1 bg-indigo-50 px-3 py-1.5 rounded-full text-sm">
                  <Shield size={14} /> 선생님 모드
                </span>
                  ...
                  {isSettingsMenuOpen && (
                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border z-50 overflow-hidden animate-fadeIn">
                      <button
                        ...
                        className="w-full text-left px-4 py-3 hover:bg-gray-50 text-sm text-gray-700 font-medium border-b flex items-center gap-2"
                      >
                      ...
                      <button
                        ...
                        className="w-full text-left px-4 py-3 hover:bg-gray-50 text-sm text-gray-700 font-medium flex items-center gap-2"
                      >
                        <Users size={14} /> 학생 관리
                      </button>
                    </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fadeIn p-4">
            <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-sm">
              <h3 className="text-xl font-bold mb-6 text-gray-800 flex items-center justify-center gap-2">
                <Lock className="text-indigo-600" /> 선생님 로그인
              </h3>
              <input
                type="password"
                value={password}
                ...
                className="w-full border-2 border-gray-200 p-3 rounded-lg mb-4 focus:outline-none focus:border-indigo-500"
                placeholder="비밀번호"
                autoFocus
              />
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-md h-[80vh] flex flex-col">
              <div className="flex justify-between items-center mb-4 border-b pb-2">
                ...
                <button onClick={() => setIsStudentManageModalOpen(false)}>
